...
<body>
        ...
        <div class="tab-content active" id="events-tab">
            <div class="section">
                ...
                <form id="eventForm">
                    ...
                    <div class="form-group"><label for="eventTimezone">Timezone:</label><select id="eventTimezone">
                    ...
                    <div class="form-group"><label for="eventFlyer">Event Flyer (Optional):</label><input type="file" id="eventFlyer" accept="image/jpeg,image/jpg,image/png,image/gif,image/webp"><small style="display:block;margin-top:0.5rem;color:#666;">Upload a flyer image (JPG, PNG, GIF, or WebP - max 16MB)</small><div id="currentFlyerPreview" style="display:none;margin-top:0.5rem;"><img id="currentFlyerImage" src="" alt="Current Flyer" style="max-width:150px;border-radius:8px;border:2px solid #ddd;"><br><button type="button" class="btn btn-danger" onclick="removeFlyer()" style="margin-top:8px;" id="removeFlyerBtn">Remove Flyer</button></div></div>
                    ...
                    <input type="hidden" id="removeFlyerFlag" value="0">
    ...
    <script>
        ...
        function renderEventsList(events) { const c = document.getElementById('eventsList'); if(!events.length) { c.innerHTML = '<p style="color:#666;text-align:center;">No farbrengens yet.</p>'; return; } let html = ''; events.forEach(e => { html += `<div class="event-item"><div class="event-item-header"><div class="event-item-title">${esc(e.title)}</div><div class="event-item-actions"><button class="btn btn-secondary" onclick="editEvent(${e.id})">Edit</button><button class="btn btn-danger" onclick="deleteEvent(${e.id})">Delete</button></div></div><div class="event-item-details">${e.farbrenger?'<strong>Farbrenger:</strong> '+esc(e.farbrenger)+'<br>':''}${e.occasion?'<strong>Occasion:</strong> '+esc(e.occasion)+'<br>':''}<strong>Date:</strong> ${fmtDT(e.event_date)} <span style="color:#6B2C3E;font-weight:600;">${getTzAbbr(e.event_timezone)}</span>${e.zoom_link?'<br><a href="'+esc(e.zoom_link)+'" target="_blank">Zoom Link</a>':''}</div></div>`; }); c.innerHTML = html; }
        function getTzAbbr(tz) { if(!tz) return 'ET'; const abbrs = {'America/New_York':'ET','America/Chicago':'CT','America/Denver':'MT','America/Los_Angeles':'PT','America/Anchorage':'AKT','Pacific/Honolulu':'HT','Asia/Jerusalem':'IST','Asia/Dubai':'GST','Europe/London':'GMT','Europe/Paris':'CET','Europe/Moscow':'MSK','Australia/Sydney':'AEST','Asia/Shanghai':'CST','Asia/Tokyo':'JST'}; return abbrs[tz] || tz.split('/').pop(); }
        ...
        document.getElementById('eventForm').addEventListener('submit', async function(e) {
            ...
            if (flyerInput.files && flyerInput.files.length > 0) { f.append('flyer_file', flyerInput.files[0]); }
            if (document.getElementById('removeFlyerFlag').value === '1') { f.append('remove_flyer', '1'); }
        ...
        function resetEventForm() { editingEventId = null; document.getElementById('eventForm').reset(); document.getElementById('eventId').value = ''; document.getElementById('eventTimezone').value = 'America/New_York'; document.getElementById('currentFlyerPreview').style.display = 'none'; document.getElementById('removeFlyerFlag').value = '0'; document.getElementById('eventFormTitle').textContent = 'Add New Farbrengen'; document.getElementById('submitEventBtn').textContent = 'Add Farbrengen'; document.getElementById('cancelEditBtn').style.display = 'none'; }
        function removeFlyer() { if(confirm('Remove this flyer?')) { document.getElementById('eventFlyer').value = ''; document.getElementById('currentFlyerPreview').style.display = 'none'; document.getElementById('removeFlyerFlag').value = '1'; } }
